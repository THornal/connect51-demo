...
collab_fwci_col = find_col(collab, ["mean_fwci","fwci"])
...
journal_fwci_col = find_col(journals, ["mean_fwci","fwci"])

sdg_inst_col = find_col(sdg, ["institution_name","institution","display_name"])
sdg_fwci_col = find_col(sdg, ["mean_fwci","fwci"])

country_col = find_col(geo, ["country","country_name","display_name"])
...
fallback_coords = {
    ...
    "France": (46.2276, 2.2137), "India": (20.5937, 78.9629),
...
if page == "Dashboard":
    ...
    with st.expander("Debug: detected columns"):
        st.write({
            "collab_inst_col": collab_inst_col,
            "collab_subject_col": collab_subject_col,
            "collab_fwci_col": collab_fwci_col,
            "journal_name_col": journal_name_col,
            "journal_subject_col": journal_subject_col,
            "journal_fwci_col": journal_fwci_col,
            "sdg_inst_col": sdg_inst_col,
            "sdg_fwci_col": sdg_fwci_col,
...
if page == "Collaboration Network":
    ...
    if not country_col or not total_col:
        st.error("Could not detect the required geography columns.")
        ...
        if network_df.empty:
            st.error("No usable latitude/longitude data found.")
            st.dataframe(geo.head(20))
        else:
